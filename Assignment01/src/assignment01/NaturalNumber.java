...
public class NaturalNumber {
	...
	public NaturalNumber times( NaturalNumber multiplicand) throws Exception{
		...
		for (int i = 0; i < multiplicand.coefficients.size(); i++){
			...
			tempRow = this.timesSingleDigit(multiplicand.coefficients.get(i)).timesBaseToThePower(i);
	...
	public NaturalNumber timesSingleDigit(int multiplicand){
		NaturalNumber product = new NaturalNumber(this.base);
		int carry = 0;
		for (int i = 0; i < this.coefficients.size(); i++){
			product.coefficients.addLast((multiplicand * this.coefficients.get(i) + carry) % base);
			carry = (multiplicand * this.coefficients.get(i) + carry)/base;			
		}
		if (carry != 0){
	...
	public NaturalNumber  minus(NaturalNumber second) throws Exception{
		...
		int borrow = 0;
		this.padNumber(second);//helper method defined at the bottom of the code
		for (int i = 0; i < this.coefficients.size(); i++){
			int a = first.coefficients.get(i);
			int b = second.coefficients.get(i);
			if (a >= b){
				borrow = 0;
			}
			else{
			...
			difference.coefficients.addLast(a + borrow - b);

		}
		
		trim(second);//defined at the bottom of the code
	...
	public NaturalNumber divide( NaturalNumber  divisor ) throws Exception{
		...
		NaturalNumber one = new NaturalNumber(1, this.base);
		while (remainder.compareTo(divisor) >= 0) {
			NaturalNumber tempDivisor = divisor.clone();
			int count = 0;
			NaturalNumber i = new NaturalNumber(0, this.base);
			...
			while (remainder.compareTo(tempDivisor) >= 0){
				...
				trim(tempDivisor);	//helper method defined at the bottom of the code
				i = i.plus(one);
			}
	...
	/**
	 * Helper method that pads second with zeroes if it had less digits than this
	 * makes this and second have the same number of digits
	 * this method is used in the minus method.
	 * @param second
	 * @return void
	 */
	private void padNumber(NaturalNumber second){
		int   diff = this.coefficients.size() - second.coefficients.size();
		while (diff > 0){  //  this is bigger
	...
	/**
	 * Helper method that trims the unnecessary zeroes from a number.
	 * @param x
	 * @return NaturalNumber
	 */
	private static NaturalNumber trim(NaturalNumber x){
		while ((x.coefficients.size() > 1) & 
				(x.coefficients.getLast().intValue() == 0)){
			x.coefficients.removeLast();
		}
		return x;
	}
}
